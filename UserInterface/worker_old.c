...
int DoWork(int dutNumber)
{
	...
	if (WorkIsReady(dutNumber)) // start testing dut
	{
	    ...
	    if(test_mode == CAPABILITY)
		{
			WriteTestResultsToFile (totalResult[dutNumber], dutNumber, GLB_FALSE, totalTestTime);  
		}
		else
		{
			WriteTestResultsToFile (totalResult[dutNumber], dutNumber, GLB_TRUE, totalTestTime);
		}
 ...
 int DoTestSequence(int dutNumber)
{
	...
	for(int i=0;i<NumOfTests;i++)
	{
		if (funcptr10[dutNumber][i] != NULL)
		{
			...
			{
				Scan (mtgTestStepInfo[dutNumber][i].testRepeat, "%s>%d", &maxRepeat);
				do
				{
					funcptr10[dutNumber][i](i+1, dutNumber);
					ProcessSystemEvents();
					if(!strcmp(mtgTestStepInfo[dutNumber][i].testType,"R"))  // check if test report is enabled
					{
					...
					if(EStopIsEnable[dutNumber])
					{
						...
						//InitSpecAn(GetTestInstrumentNumber(dutNumber));			   // reset spectrum analyzer after ESTOP
						return 0;
					}
					...
					if(!strcmp(mtgTestStepInfo[dutNumber][i].testResult,"Pass"))
						break;
					attempt++;


				}while(attempt<maxRepeat+1);
...
{
	if(ITACIsEnable &&  ExitStatus == GLB_FALSE)
	{
		int error, status;
