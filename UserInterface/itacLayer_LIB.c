...
APILONG __stdcall ewMII_init (char * errorString)
{
	...
	GetProjectDir(dir);
	strcat(dir, "ewMII\\ewMII.options");
	
	for(int loop_count=0; (result != 0) && (loop_count < 2); loop_count++)
	{
    	//result=apiInit(tempStr,&errorString);
		result=apiInit(NULL,&errorString);  // "C:/ewMII/ewMII.options"
	}
...
APILONG __stdcall upResultDataAndRecipe(int totalResult[100],int itacResult[100], int store_type, char* SerialNumber,TestStepInfoStruct  (*TestStepInfo)[10][150],int nestNum, float testTime, int numTests, char * errorString)
{
	...
	if (itacResult[nestNum] == 0)  //unit is valid for this station
	{
		...
		else
		{
			...
			{  // store type: 0-only state, 1- only fail data 2-all data up to and including first fail, 3 - all if pass, fail only if fail, 4 all data 
...
APILONG __stdcall upItacDataAndRecipe(int totalResult[100],int itacResult[100], int store_type, char* SerialNumber,ItacTestStepStruct  (*ItacTestStep)[10][150],int nestNum, float testTime, int numTests, char * errorString)
{
	...
	char productRev[100]={0};
	char testName[numTests][200];  
	struct ResultDataExtendedStruct resultDataExtArray[numTests];
	int testIndex = 0;
	...
	if (itacResult[nestNum] == 0)  //unit is valid for this station
	{
		...
		else
		{
			...
			{  // store type: 0-only state, 1- only fail data 2-all data up to and including first fail, 3 - all if pass, fail only if fail, 4 all data 
				if( ((*ItacTestStep)[nestNum][i].testResult[0] == 0) || // Don't store blank results (don't know how this would happen) 
					((*ItacTestStep)[nestNum][i].testSkip[0] == 'Y') ||	// Don't store skipped tests
					((*ItacTestStep)[nestNum][i].testType[0] == 'R') ||	// Dont store "report" type tests
					((*ItacTestStep)[nestNum][i].testResult[0] == 'P' && store_type == 1) || // Dont store passes if store_type == 1 (fail only)
					(serialNumberState && (*ItacTestStep)[nestNum][i].testResult[0] == 'P' && store_type == 3) )// store_type 3 == all if pass, fail only if fail	
						continue;	
			
				resultDataExtArray[testIndex].nr = i+1;
				//sprintf(testName[i],"T%d_%s",i+1,(*ItacTestStep)[nestNum][i].testName);
				//resultDataExtArray[testIndex].name = testName[i];
				resultDataExtArray[testIndex].name = (*ItacTestStep)[nestNum][i].testName;
				resultDataExtArray[testIndex].value = (*ItacTestStep)[nestNum][i].testResultVal;
				resultDataExtArray[testIndex].min = (*ItacTestStep)[nestNum][i].testLowLimit;
				resultDataExtArray[testIndex].max = (*ItacTestStep)[nestNum][i].testHighLimit; 
				resultDataExtArray[testIndex].nom = "";
				resultDataExtArray[testIndex].toleranz = "";
				resultDataExtArray[testIndex].unit = (*ItacTestStep)[nestNum][i].testUnit; 
				resultDataExtArray[testIndex].failCode = (strcmp((*ItacTestStep)[nestNum][i].testResult, "Pass") != 0); // 0 PASS, 1 FAIL
				resultDataExtArray[testIndex].messtyp = "0";
				testIndex++;
		
				if(store_type == 2 && (*ItacTestStep)[nestNum][i].testResult[0] != 'P')
					break;
			} // end for loop
			result = uploadResultDataAndRecipe(sessionID, sITACStationId, processLayer, SerialNumber, serialNumberRefPos, serialNumberState, productNo, productRev, bookDate, testTime, testIndex, resultDataExtArray, &errorString);
		}
	}
...
#if 0
static ITAC_result_cnt[10]={0};
add_ITAC_result(int nestNum, int testNumber, char *param_name, void *value, void *limit_low, void *limit_high, char *units, int data_type)
{
	if(data_type == VAL_INTEGER)
	{
		sprintf(ITAC_results[nestNum][ITAC_result_cnt].value, "%d", *(int *) value)
		sprintf(ITAC_results[nestNum][ITAC_result_cnt].limit_low, "%d", *(int *) limit_low)
		sprintf(ITAC_results[nestNum][ITAC_result_cnt].limit_high, "%d", *(int *) limit_high)
	}
	if(data_type == VAL_DOUBLE)
	{
		sprintf(ITAC_results[nestNum][ITAC_result_cnt].value, "%lf", *(double *) value)
		sprintf(ITAC_results[nestNum][ITAC_result_cnt].limit_low, "%lf", *(double *) limit_low)
		sprintf(ITAC_results[nestNum][ITAC_result_cnt].limit_high, "%lf", *(double *) limit_high)
	}
	if(data_type == VAL_CHAR)
	{
		sprintf(ITAC_results[nestNum][ITAC_result_cnt].value, "%s", (char *) value)
		sprintf(ITAC_results[nestNum][ITAC_result_cnt].limit_low, "%s", (char *) limit_low)
		sprintf(ITAC_results[nestNum][ITAC_result_cnt].limit_high, "%s", (char *) limit_high)
	}
	ITAC_result_cnt++;
}
...
freq_x_over()
{
	...
	add_ITAC_result(test_number, "Band0_Low_FrequencyMHz", (void*) &Band0_Low_FrequencyMHz, (void*) &low_limit, (void*) &high_limit, VAL_INTGER);
	add_ITAC_result(test_number, "Band0_High_FrequencyMHz",(void*) &Band0_High_FrequencyMHz, (void*) &low_limit, (void*) &high_limit, VAL_INTGER);
